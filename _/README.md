...
> | Tue, Feb 24, 2026 | 9 | 13 | Continued with **Example 11** (showed how $T$ is non-singular, but not onto; and still has a left inverse), looked at **Example 12** ($T=(x_1,x_2) = (x_1 + x_2,x_1)$, non-singular, onto, has inverse), stated and proved **Theorem 9** (when dimensions are same and finite, for a linear transformation $T$, (i) invertibility, (ii) non-singularity, (iii) onto-ness are equivalent), proved also the extended version, Theorem 9 (that says (iv) if $\{\alpha_i\}$ is a basis then $$ Completed till Theorem 11 | 
